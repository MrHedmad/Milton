    ...
    async def on_message(self, message: discord.Message):
        ...
        for attachment in message.attachments:
            ...
            try:
                ...
                async with self.bot.http_session.request("GET", attachment.url) as stream:
                    bytes = await stream.read()
                    file_metadata = PdfReader(BytesIO(bytes))
                
                convert_ptl = partial(convert_from_bytes, pdf_file = bytes, single_file=True, fmt="png")
            ...
            embed = discord.Embed()
            embed.set_image(url = "attachment://image.png").set_footer(text = "Automatic PDF preview rendering")

            info = file_metadata.getDocumentInfo()
            file_len = file_metadata.getNumPages()

            if info.title:
                embed.add_field(name = "Title", value = info.title.encode("ascii").decode("UTF-8"))
